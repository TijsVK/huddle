...
- **Endpoint-coverage note:** the plugin inspects the container/volume/exec/service create factories. Lower-priority uninspected: `/containers/{id}/update` (self-DoS only), docker `/plugins/*` install (needs egress; future work), `/build` insecure entitlements (refused by dockerd — `--allow-insecure-entitlement` unset).
