...
- **#3 symlink variant CLOSED** (`ln -s /proc/sys /work/evil; -v /work/evil:/x`): shared workspace/folder mounts are remounted `nosymfollow` in the sidecar so dockerd can't follow a symlink out of the workspace during bind-source resolution. Verified live; compose `./path` binds unaffected. Requires Linux ≥ 5.10.
- **THREE adversarial review rounds done — each found real host escapes, all fixed + verified live.** Round 3 (of the hardened authz plugin) found two more /proc/sys bind-in-disguise doors: (#1 CRITICAL) `/volumes/create` was unauthorized — a named `local` volume with `device=/proc/sys,o=bind` referenced by name dodged the create-time guard (→ now authorize /volumes/create, `validateVolumeCreate`); (#2 HIGH) swarm `/services/create` is an unchecked container/mount factory (→ deny swarm services). Also added `/run/docker/plugins` to the sensitive bind roots. Escape e2e now 19 assertions, all green.
- **FOUR review rounds done.** Round 4 found two more live escapes: (#2 HIGH) the round-3 nosymfollow covered only shared mounts, so binding a NON-shared sidecar dir (`/tmp`,`/etc`) + planting a symlink (`/tmp/x -> /proc`) still escaped → fixed by switching binds to an ALLOWLIST (only named-volume / docker.sock / under a nosymfollow'd `safeRoot` = workspace+folder-mappings, passed gateway→plugin); (#1 CRITICAL) `docker plugin install` POSTs a local tar with host mounts/caps, runs as root on the sidecar → deny `/plugins/*`. Both verified live; compat preserved (workspace binds, named volumes, Ryuk socket). Escape e2e now 22 assertions.
- **Round 5 (final) came back CLEAN** — no new host-root escape. Re-audited /build (no insecure-entitlement → build steps unprivileged, no host mount), /containers/archive (chrootarchive/securejoin), /images/load (graph-root chroot), /networks/create (sidecar netns only), /update (cgroup resources only), and confirmed safeRoots can't be attacker-widened (folder-mappings are operator-token-only; sensitive-root deny runs before the safeRoot loop, so even a `/` safeRoot admits nothing). Closure rests on: dockerd keeps no `--allow-insecure-entitlement`, the operator token is never in a devcontainer env, docker≥28 extraction hardening.
- **Reduced bind surface:** nested containers can no longer bind arbitrary sidecar paths (`/etc`,`/tmp`), only workspace/folder-mapping/named-volume/docker.sock. Deliberate (those dirs are symlink-plant ground on the privileged sidecar).
- **Endpoint-coverage note:** inspected factories: containers/create, exec, volumes/create, services (deny), plugins (deny). Lower-priority uninspected: `/containers/{id}/update` (self-DoS only), `/build` insecure entitlements (refused by dockerd — `--allow-insecure-entitlement` unset).
