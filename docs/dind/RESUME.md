...
- **Open residual:** the #3 host-kernel-bind deny is lexical; a workspace-symlink variant (`ln -s /proc/sys /work/evil; -v /work/evil:/x`) can still reach it. Needs a `nosymfollow` workspace mount or dropping host-path binds. Documented in SECURITY-CRITICAL.md. Next-highest-priority item.
