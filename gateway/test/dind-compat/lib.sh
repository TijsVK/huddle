...
up() {
  ...
  local sock="${PREFIX}-${name}-sock" data="${PREFIX}-${name}-data"
  ...
  docker run -d --name "$dind" --privileged \
    ...
    -v "$sock":/var/run/dind -v "$data":/var/lib/docker \
...
down() {
  ...
  docker volume rm "${PREFIX}-${name}-sock" "${PREFIX}-${name}-data" >/dev/null 2>&1 || true
}
