...
export HUDDLE_SYSBOX=1 HUDDLE_IMAGE=huddle-local:dind HUDDLE_NO_PULL=1 \
       BASE_IMAGE_VSCODE=huddle-e2e-base-sysbox:latest HUDDLE_RUNTIME=docker HUDDLE_PORT=$PORT
...
curl -s -H "$AUTH" -H 'content-type: application/json' -X POST "$API/api/docker/start" \
  -d "{\"imageName\":\"huddle-e2e-base-sysbox:latest\",\"containerName\":\"$DC\",\"ideName\":\"vscode\",\"empty\":true}" >/dev/null
