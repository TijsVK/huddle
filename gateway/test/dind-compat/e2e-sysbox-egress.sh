...
PORT=3997; DC=e2e-sbx-egress; rc=0
...
node "$REPO/cli/dist/index.js" init >/tmp/nege-init.log 2>&1 && pass "gateway init (SYSBOX)" || { fail "init"; cat /tmp/nege-init.log>&2; exit 1; }
...
resp=$(curl -s -H "$AUTH" -H 'content-type: application/json' -X POST "$API/api/docker/start" -d "{\"imageName\":\"huddle-e2e-base-sysbox:latest\",\"containerName\":\"$DC\",\"ideName\":\"vscode\",\"empty\":true}")
