...
#
# Requires locally-built huddle-local:dind + huddle-e2e-base-sysbox:latest.
set -uo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; REPO="$(cd "$HERE/../../.." && pwd)"
PORT=3969; DC=e2e-sbx-aspire; rc=0
...
export HUDDLE_SYSBOX=1 HUDDLE_IMAGE=huddle-local:dind HUDDLE_NO_PULL=1 BASE_IMAGE_VSCODE=huddle-e2e-base-sysbox:latest HUDDLE_RUNTIME=docker HUDDLE_PORT=$PORT
node "$REPO/cli/dist/index.js" init >/tmp/sbx-aspire-init.log 2>&1 && pass "gateway init (SYSBOX)" || { fail init; cat /tmp/sbx-aspire-init.log>&2; exit 1; }
...
curl -s -H "$AUTH" -H 'content-type: application/json' -X POST "$API/api/docker/start" -d "{\"imageName\":\"huddle-e2e-base-sysbox:latest\",\"containerName\":\"$DC\",\"ideName\":\"vscode\",\"empty\":true}" >/dev/null
